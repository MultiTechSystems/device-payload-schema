// TS013 Payload Codec — decentlab_dl_alb
// Schema version: 3
// Generated from: dl-alb.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var incoming_radiation = readS(buf, pos, 2, endian);
      pos += 2;
      vars.incoming_radiation = incoming_radiation;
      d.incoming_radiation = (incoming_radiation / 10);
      var reflected_radiation = readS(buf, pos, 2, endian);
      pos += 2;
      vars.reflected_radiation = reflected_radiation;
      d.reflected_radiation = (reflected_radiation / 10);
      d.albedo = (d.incoming_radiation > 0 && d.reflected_radiation >= 0) ? (d.incoming_radiation !== 0 ? d.reflected_radiation / d.incoming_radiation : NaN) : 0;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.incoming_radiation !== undefined || d.reflected_radiation !== undefined || d.albedo !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeS(buf, pos, 2, Math.round(((d.incoming_radiation !== undefined ? d.incoming_radiation : 0) * 10)), endian);
      pos += 2;
      writeS(buf, pos, 2, Math.round(((d.reflected_radiation !== undefined ? d.reflected_radiation : 0) * 10)), endian);
      pos += 2;
      // skip computed field albedo
    }
    if (_flags & (1 << 1)) {
