// TS013 Payload Codec — decentlab_dl_5tm
// Schema version: 3
// Generated from: dl-5tm.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var dielectric_permittivity = readU(buf, pos, 2, endian);
      pos += 2;
      vars.dielectric_permittivity = dielectric_permittivity;
      d.dielectric_permittivity = (dielectric_permittivity / 50);
      d.volumetric_water_content = (((4.3e-06 * d.dielectric_permittivity - 0.00055) * d.dielectric_permittivity + 0.0292) * d.dielectric_permittivity - 0.053);
      var soil_temperature_raw = readU(buf, pos, 2, endian);
      pos += 2;
      vars.soil_temperature_raw = soil_temperature_raw;
      d.soil_temperature_raw = soil_temperature_raw;
      d.soil_temperature = ((d.soil_temperature_raw + -400) / 10);
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.dielectric_permittivity !== undefined || d.volumetric_water_content !== undefined || d.soil_temperature_raw !== undefined || d.soil_temperature !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round(((d.dielectric_permittivity !== undefined ? d.dielectric_permittivity : 0) * 50)), endian);
      pos += 2;
      // skip computed field volumetric_water_content
      writeU(buf, pos, 2, Math.round((d.soil_temperature_raw !== undefined ? d.soil_temperature_raw : 0)), endian);
      pos += 2;
      // skip computed field soil_temperature
    }
    if (_flags & (1 << 1)) {
