...
# result.data == {"temperature": 23.45, "humidity": 65, "battery": 3300}
```
...
result = interpreter.encode({
    "command": 0x01,   # or the label "set_interval"
...
│                    DEVICE FIRMWARE                      │
├────────────────────────────────────────────────────────┤
│  Generic Codec Library (~18KB stripped, -Os)           │
│  ├── schema_encode()                                   │
│  ├── schema_decode()                                   │
│  └── schema_load_binary()                              │
├────────────────────────────────────────────────────────┤
...
Decode, measured by `make bench-c` on a 29-byte, 15-field Decentlab frame
(`decentlab/dl-lid`), schema built once outside the loop:

| Operation | Python | C |
|-----------|--------|---|
| Decode | ~25 µs | ~0.12 µs |

Encode and binary schema load have no benchmark in the repository; figures depend on the
machine, so re-run the target rather than quoting these.
...
| **Encode** | value → bytes | (value - add) × div ÷ mult, then rounded to an integer |
...
// Binary schemas (python3 tools/schema_binary.py encode <schema.yaml> -o <out.bin>)
// Uplink: the sensor_uplink schema above
static const uint8_t UPLINK_SCHEMA[] = {
    ...
    0x02, 0x00, 0xF9, 0x89,        // battery: u16
};

// Downlink: command u8 + parameter u16 (the device_config schema without its lookup)
static const uint8_t DOWNLINK_SCHEMA[] = {
    0x50, 0x53, 0x01, 0x00, 0x02,  // Header
    0x01, 0x00, 0x42, 0x8C,        // command: u8
    0x02, 0x00, 0x14, 0xFE,        // parameter: u16
};
...
int create_uplink(float temp, float hum, uint16_t bat, uint8_t* buf) {
    ...
    encode_inputs_add_double(&inputs, "battery", bat);
...
- [Binary Schema Format](../tools/schema_binary.py) - Binary encoding tool (`encode`, `decode`, `dump`, `info`)
